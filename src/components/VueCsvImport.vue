  ...
  setup(props, { emit }) {
    let VueCsvImportData = reactive({
      ...
      firstSampleRow: computed(() =>
        isArray(VueCsvImportData.csvSample) &&
        isArray(VueCsvImportData.csvSample[0])
          ? VueCsvImportData.csvSample[0]
          : null
      ),
      allFieldsAreMapped: computed(() =>
        every(VueCsvImportData.fields, function (field) {
          return (
            typeof VueCsvImportData.map[field.key] !== "undefined" ||
            !field.required
          );
        })
      ),
    ...
    const buildMappedCsv = function () {
      let newCsv = VueCsvImportData.fileHasHeaders
        ? VueCsvImportData.rawCsv
        : drop(VueCsvImportData.rawCsv);
